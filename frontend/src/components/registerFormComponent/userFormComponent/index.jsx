...
const UserFormComponent = (props) => {
   ...
   useEffect(() => {
      ...
      const getuser = async () => {
         try {
            if (props.operation === 'update' || props.operation === 'view') {
               const { data } = await api.get(`/users/get/${props.id}`);
            ...
            setMsg('Erro ao buscar usuário');
            timer();
         }
      }
   ...
   const registerOurUpdate = async (e, operation) => {
      try {
         ...
         if (operation === "update") {
            result = await api.put(`/users/updateUser/${props.id}`, {
               name: name,
               email: email,
               password: password,
               permissions: name_permission,
